...
<ul>
  <li>$m$ variables with <b>mean</b> $\mathcal{O}(\frac{1}{m})$ (the $\big( \frac{\partial y}{\partial a_i}\big)^2$). These go to 0 as $m \to \infty$, so we can just ignore them. </li>
  <li>Another $m$ variables with <b>mean</b> $\mathcal{O}(1)$ (the $\big( \frac{\partial y}{\partial b_i}\big)^2$).</li>
</ul>

Notice that the constant mean terms, $\big( \frac{\partial y}{\partial b_i}\big)^2 = \sigma^2 (a_i x)$ are actually iid random variables, because the $a_i$ are iid (remember that $x$ is not random, but just a fixed value). We can pull out a $\sqrt m$ factor from under the square root and get this:

$$\Vert \nabla y \Vert \sim \sqrt{m} \sqrt{\frac{1}{m} (\sigma^2 (a_1 x) + \dots + \sigma^2(a_m x))}$$

The term in the square root is a nice empirical average of iid random variables. By the law of large numbers, it approaches the true average as $m \to \infty$. This true average is a constant as we scale $m$, and more importantly <b>deterministic</b>. So the gradient norm itself approaches a deterministic value which goes as $\sqrt{m}$. This means that,

$$\color{green} \Vert \nabla y \Vert \sim \mathcal{O}(\sqrt{m}) \text{ and } \Vert \nabla y \Vert \sim \Omega(\sqrt{m})$$

<p>Recall that big-$\Omega$ is the same as big-O, but an asymptotic lower bound instead of an upper bound. We need a lower bound for the gradient norm because it is in the denominator of the expression for $\kappa$ (which we are trying to upper bound).</p>

We can empirically verify this asymptotic behavior pretty easily:
...
<figure style="margin: auto auto; text-align: center; width:100%" vertical-align='middle'>
    ...
    <figcaption>Norm of the gradient of a single hidden layer tanh network at initialization as the width increases. The input value $x$ is fixed. As expected, it grows as $\sim \sqrt m$.</figcaption>
</figure>
